...
from .dampf_structures import RayEvolutionOperatorRefs, mpo_list
from .dampf_utils import boltzmann_occupation
...
def local_change_basis_tensor(map_matrix, basis_site, mode):
    """
        Build one local map tensor in the propagation basis.
    """
...
def build_local_tensors(params, pseudomodes, basis_data):
    """
        Build all one-site tensors used in local propagation.
    """
    dt = params.dt
...
def build_system_propagator(Hs, dt):
    """
        Build the system propagator.
    """
    return expm(-1.0j * Hs * dt)


@ray.remote(num_cpus=1)
def build_propagator_mpos(params, pseudomodes, basis_data, system_index_maps):
    """
        Build the local propagation MPOs used in one time step.
    """
    site_iden, site_Uv, site_Diss, site_Usv_left, site_Usv_right, site_Usv_both = build_local_tensors(params, pseudomodes, basis_data)
    ...
    UsvMPO = mpo_list(pseudomodes, length=num_local_mpos, rank=1)
    ...
    for ind in range(num_local_mpos):
        ...
        UsvMPO[ind] = mp.MatrixProductObject(Usv_chain)

    local_update_mpos_1 = mpo_list(pseudomodes, length=num_local_mpos, rank=1)

    if params.trotter_order == "first":
        local_update_mpos_2 = None
    else:
        local_update_mpos_2 = mpo_list(pseudomodes, length=num_local_mpos, rank=1)

    for ind in range(num_local_mpos):
        local_step = mp.apply_mpo(UsvMPO[ind], DissMPO)
        local_update_mpos_1[ind] = mp.apply_mpo(UvMPO, local_step)

        if params.trotter_order == "second":
            local_update_mpos_2[ind] = mp.apply_mpo(DissMPO, UsvMPO[ind])
...
def prepare_ray_operator_refs(params, system, pseudomodes, basis_data, system_index_maps):
    """
    ...
    local_mpo_refs = build_propagator_mpos.options(num_returns=num_local_mpo_returns).remote(params, pseudomodes, basis_data, system_index_maps)
    ...
    return RayEvolutionOperatorRefs(
        Us=build_system_propagator.remote(system.Hs, params.dt),
...
def print_system_summary(system, system_index_maps):
    """
        Print a compact summary of the prepared system.
    """
    ...
    print(f"first stored indices    : {system_index_maps.single_to_double_indices[:min(10, system_index_maps.num_stored_indices)]}")
