...
from .dampf_structures import BasisData


def basis_initialization(pseudomodes, vectorized=False):
    """
        Build the identity-plus-traceless operator basis in matrix and vectorized form.
    """
    basis = [[np.zeros((pseudomodes.fock_dim[n][q] ** 2, pseudomodes.fock_dim[n][q], pseudomodes.fock_dim[n][q]), dtype=float) for q in range(pseudomodes.Q[n])] for n in range(pseudomodes.N)]
    vecbasis = [[np.zeros((pseudomodes.fock_dim[n][q] ** 2, pseudomodes.fock_dim[n][q] ** 2), dtype=float) for q in range(pseudomodes.Q[n])] for n in range(pseudomodes.N)]
...
def fock_to_idtr(pseudomodes, idtr_basis):
    ...
    """
    transform = [[np.zeros((pseudomodes.fock_dim[n][q] ** 2, pseudomodes.fock_dim[n][q] ** 2), dtype=float) for q in range(pseudomodes.Q[n])] for n in range(pseudomodes.N)]
...
def prepare_basis(pseudomodes):
    ...
    """
    basis, vecbasis = basis_initialization(pseudomodes, vectorized=True)
