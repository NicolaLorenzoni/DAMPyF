...
if [ -n "${SLURM_MEM_PER_NODE:-}" ]; then
    echo "Memory per node:                      $(awk "BEGIN {printf \"%.4f\", ${SLURM_MEM_PER_NODE}/1024}") GB"
else
...
#### LAUNCHING CODE ######################################
# In configure_simulation.py, set execution_mode = "slurm" for this batch script.
python run_dampf.py > ${SLURM_JOB_NAME}_${SLURM_JOB_ID}.log 2>&1
#########################################################
