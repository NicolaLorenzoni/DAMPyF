...
class MatrixProductObject:
    ...
    def load(cls, source):
        ...
        if "num_sites" in source.attrs:
            num_sites = int(source.attrs["num_sites"])
        else:
            num_sites = int(source.attrs["len"])

        local_tensors = []
        for site in range(num_sites):
            local_tensors.append(source[str(site)][()])

        if "left_canonical" in source.attrs and "right_canonical" in source.attrs:
            canonical_form = (int(source.attrs["left_canonical"]), int(source.attrs["right_canonical"]))
        else:
            canonical_form = tuple(int(value) for value in source.attrs["canonical_form"])
    ...
    def compress(self, method="svd", rank=None, relerr=None, direction=None, canonicalize=True, svd_method="standard"):
        """
            Compress this object in place using the selected SVD backend.
        """
        if method != "svd":
            raise ValueError("Only SVD compression is supported in dampf_mps_operations.")
        if len(self) == 1:
            return
    ...
    def _compress_left_to_right(self, rank, relerr, svd_method):
        for site in range(len(self) - 1):
            ...
            self._local_tensors[site + 1] = _matrix_dot(singular_values[:kept_rank, None] * vh_matrix[:kept_rank, :], self._local_tensors[site + 1])
            self.left_canonical = site + 1
            self.right_canonical = max(self.right_canonical, site + 2)
    ...
    def _compress_right_to_left(self, rank, relerr, svd_method):
        for site in range(len(self) - 1, 0, -1):
            ...
            self._local_tensors[site - 1] = _matrix_dot(self._local_tensors[site - 1], u_matrix[:, :kept_rank] * singular_values[None, :kept_rank])
...
def maximum_bond_dimensions(local_dimensions):
    ...
    """
    flattened_dimensions = np.array([int(np.prod(local_dimension)) for local_dimension in local_dimensions], dtype=object)
    bond_dimensions = []
...
def _local_dot(left_tensor, right_tensor, axes):
    ...
    result = np.rollaxis(result, left_tensor.ndim - contracted_legs_left, left_tensor.ndim + right_tensor.ndim - contracted_legs_left - contracted_legs_right - 1)
...
def _local_add(local_tensors):
    ...
    result_shape = (sum(tensor.shape[0] for tensor in local_tensors),) + physical_shape + (sum(tensor.shape[-1] for tensor in local_tensors),)
...
def _normalize_ranks(sites, rank, local_dimensions, force_rank):
    ...
    if not force_rank:
        ranks = tuple(min(requested_rank, maximal_rank) for requested_rank, maximal_rank in zip(ranks, maximum_bond_dimensions(local_dimensions)))
